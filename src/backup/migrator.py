...
def migrate_media(
    ...
    Returns:
        {total: N, hardlinked: N, copied: N, skipped: N, errors: [str]}
    """
    ...
    stats = {'total': 0, 'hardlinked': 0, 'copied': 0, 'skipped': 0, 'errors': []}
    ...
    for i, ref in enumerate(all_refs):
        ...
        if on_progress:
            on_progress(os.path.basename(src), i + 1, len(all_refs))
