...
def run_backup(
    ...
    try:
        ...
        if not keys:
            ...
            try:
                ...
                result['errors'].append(
                    ...
                    f'请确保微信正在运行，然后在 Web 页面执行"密钥提取"或运行 wechat_exp.exe keys')
                _stop_harvester(harvester_stop, harvester_thread, harvester_result)
                return result
            if not keys:
                result['errors'].append(
        ...
        decrypted = decrypt_for_backup(
            db_storage_path, output_dir, keys,
            on_progress=lambda d, p: on_progress('decrypt', d, p * 0.8 + 0.2)
        )
    ...
    try:
        ...
        on_progress('migrate',
            f"Media: {media_stats.get('hardlinked', 0)} hardlinked, {media_stats.get('copied', 0)} copied",
            1.0)
    except Exception as e:
