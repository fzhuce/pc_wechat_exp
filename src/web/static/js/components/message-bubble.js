...
class MessageBubble extends Component {
  ...
  _renderBubble(m) {
    ...
    const tag = MSG_TYPE_LABELS[m.msg_type];
    const cssCls = MSG_TYPE_CSS[m.msg_type] || '';
    const tagHtml = tag && m.msg_type !== 1 ? `<span class="msg-type-tag ${cssCls}">${tag}</span>` : '';
  ...
  _renderDetail(m) {
    ...
    case 49: {
      ...
      if (rt === 'pat') {
        ...
        const fileUrl = api.hardlinkMediaUrl(m.media_info, 6);
        if (fileUrl) {
          body = `<a href="${fileUrl}" download target="_blank" class="nontext-placeholder file-plc" style="text-decoration:none;color:inherit;cursor:pointer;display:block" onclick="event.stopPropagation()"><span>📎</span><span>${escapeHtml(p.title||'文件')}</span>${fileSize?`<div style="color:#484f58;font-size:10px;margin-top:4px">${fileSize}</div>`:''}</a>`;
        } else {
