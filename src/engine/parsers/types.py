...
def parse_link(xml_bytes):
    ...
    if root is not None:
        ...
        if appmsg is not None:
            ...
            if msg_type == 51:
                display_title = title if title and '不支持' not in title else des
                return {
                    'text': f'[视频号] {display_title}' if display_title else '[视频号]',
                    'title': display_title or '视频号',
                    'des': des or None,
                    'url': url or None,
                    'app_type': 51,
                    'render_type': 'finder',
